...
import { QUEUE_ORDER_GENERIC } from "./constants";
...
export class ConsumerOrder {
  ...
  private async consumeMessages() {
    try {
      console.log(
        `Aguardando mensagens na fila generico: ${QUEUE_ORDER_GENERIC}`
      );

      // Consumir mensagens da fila 1 (Error Queue)
      this.channel.consume(QUEUE_ORDER_GENERIC, (msg) => {
        if (msg) {
          console.log(
            `Fila ${QUEUE_ORDER_GENERIC}: Mensagem recebida: ${msg.content.toString()}`
          );
